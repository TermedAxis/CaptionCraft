            ...
            return (
              <div key={i} className="border border-gray-200 dark:border-gray-700 rounded-xl p-4 space-y-3">
                <div className="flex items-center gap-2">
                  <span className="w-6 h-6 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded-full flex items-center justify-center text-xs font-bold shrink-0">
                    {slide.slideNumber}
                  </span>
                  <p className="text-sm font-semibold text-gray-900 dark:text-white">{slide.heading}</p>
                </div>
                <ul className="space-y-1">
                  {slide.bulletPoints?.map((point, j) => (
                    <li key={j} className="flex items-start gap-1.5 text-sm text-gray-700 dark:text-gray-300">
                      <ChevronRight className="w-3.5 h-3.5 text-blue-500 dark:text-blue-400 mt-0.5 shrink-0" />
                      {point}
                    </li>
                  ))}
                </ul>
      ...
      {result.designTips?.length > 0 && (
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h4 className="text-sm font-semibold text-gray-900 dark:text-white">Design Tips</h4>
          </div>
