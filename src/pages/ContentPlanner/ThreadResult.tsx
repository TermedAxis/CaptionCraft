  ...
  return (
    <div className="space-y-5">
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
        <h3 className="font-semibold text-gray-900 dark:text-white text-base mb-1">{result.title}</h3>
        {result.threadSummary && (
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{result.threadSummary}</p>
        )}
              ...
              <div key={i} className="relative flex gap-3">
                <div className="flex flex-col items-center shrink-0">
                  ...
                  {!isLast && (
                    <div className="flex flex-col items-center mt-1 mb-1 flex-1">
                      <div className="w-0.5 flex-1 bg-gray-200 dark:bg-gray-700 min-h-[12px]" />
                ...
                <div className={`flex-1 pb-4 ${isLast ? "" : ""}`}>
                  <div className={`rounded-xl p-3.5 ${
                    ...
                    {isHook && (
                      <span className="text-xs font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wide block mb-1">Hook Tweet</span>
                    )}
                    {isClosing && (
                      <span className="text-xs font-semibold text-gray-400 uppercase tracking-wide block mb-1">Closing Tweet</span>
                    )}
                    <p className={`text-sm leading-relaxed whitespace-pre-wrap ${isClosing ? "text-white" : "text-gray-800 dark:text-gray-200"}`}>
                      {tweet.content}
                    </p>
                    <div className={`flex justify-end mt-2 text-xs ${
                      isClosing ? "text-gray-400" : tweet.charCount > 280 ? "text-red-500 font-medium" : "text-gray-400 dark:text-gray-500"
                    }`}>
                      {tweet.charCount}/280
                    </div>
                  </div>
      ...
      {result.engagementTips?.length > 0 && (
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h4 className="text-sm font-semibold text-gray-900 dark:text-white">Engagement Tips</h4>
          </div>
